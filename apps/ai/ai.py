...
collection.add(
    ids=[
        ...
        "pop_hist_population",
        "pop_hist_population_by_commune",
    ...
    documents=[
        "act_t1_employment_status_2022: Répartition par statut d'emploi (CHECK IN: 'Ensemble','Salaries','Non-salaries'). Colonnes : employed_count , total des personnes en emploi), percent_total (% de la population totale), percent_part_time (% en temps partiel), percent_women (% de femmes). Une ligne par employment_status pour 2022. Utile pour comparer salariés vs non‑salariés, y compris temps partiel et féminisation.",
        ...
        "pop_hist_population: Historique de la population totale de la commune de 1876 à 2022. Clé : indicator (valeur fixe 'Population'). Préférer cette table pour un historique de population sur une commune. Valeurs : y1876, y1881, y1886, y1891, y1896, y1901, y1906, y1911, y1921, y1926, y1931, y1936, y1954, y1962, y1968, y1975, y1982, y1990, y1999, y2006, y2007, y2008, y2009, y2010, y2011, y2012, y2013, y2014, y2015, y2016, y2017, y2018, y2019, y2020, y2021, y2022 (INT). Série temporelle de la population totale.",
		"pop_hist_population_by_commune: Historique de la population par commune de 1876 à 2022. Clé : commune_name (NOM DE LA COMMUNE). Préférer cette table pour un historique de population sur plusieurs commune ou sur le département entier. Valeurs : y1876, y1881, y1886, y1891, y1896, y1901, y1906, y1911, y1921, y1926, y1931, y1936, y1954, y1962, y1968, y1975, y1982, y1990, y1999, y2006, y2007, y2008, y2009, y2010, y2011, y2012, y2013, y2014, y2015, y2016, y2017, y2018, y2019, y2020, y2021, y2022 (INT). Série temporelle pour chaque commune.",
...
def _pipeline_internal(question_text: str, max_communes: Optional[int], override_codes: Optional[List[str]]):
    ...
    try:
        ...
        candidate_names = [name for group in results.get('ids', []) for name in group]
        _log_step(f"[RAG] Candidats tables: {', '.join(candidate_names) if candidate_names else '—'}")
        ddl_statements = ""
        for table_names in results.get("ids", []):
            for table_name in table_names:
                table_def = get_table_def(table_name)
