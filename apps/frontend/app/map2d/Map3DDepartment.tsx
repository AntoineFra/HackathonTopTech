        ...
        const fetchCityAndCenter = async () => {
            try {
                ...
                const response = await fetch(
                    `${backendUrl}/trois-d/cities/${selectedCityName}`,
                );
                if (!response.ok) {
                ...
                if (controlsRef.current) {
                    ...
                    console.log(
                        `📷 Caméra repositionnée au-dessus de ${selectedCityName}`,
                    );
                }
...
export default function Map3DDepartment() {
    ...
    return (
        <div className="relative h-screen w-full bg-black">
            ...
            {/* Info */}
            <div className="absolute top-4 left-4 min-w-[280px] rounded-lg bg-white p-4 shadow-lg">
                <h2 className="text-lg font-bold">Département 06</h2>
                <p className="text-sm text-gray-600">{cityCount} communes</p>

                {selectedCity && (
                    <div className="mt-3 rounded border-2 border-orange-500 bg-orange-100 p-3">
                ...
                <button
                    ...
                    className="mt-3 w-full rounded bg-blue-600 px-4 py-2 text-sm font-medium text-white transition-colors hover:bg-blue-700"
                >
                    Recentrer la vue
                </button>
            </div>
