...
function isInAlpesMaritimes(city: any): boolean {
    ...
    if (BLACKLIST.includes(city.name)) {
        console.log(
            `❌ ${city.name} - dans la liste noire (coordonnées aberrantes ou homonyme)`,
        );
        return false;
    }
    ...
    const isInBounds =
        lat >= 43.4 &&
        lat <= 44.5 && // Latitude raisonnable
        lng >= 6.5 &&
        lng <= 7.8; // Longitude raisonnable

    if (!isInBounds) {
        console.log(
            `❌ ${city.name} (code 06) hors limites: lat=${lat.toFixed(4)}, lng=${lng.toFixed(4)}`,
        );
    }
...
console.log(
    `✅ ${Object.keys(cityPolygons).length} villes chargées avec leurs polygones`,
);
