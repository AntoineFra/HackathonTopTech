...
function extractCount(question: string): number {
    ...
    const numberMatch =
        lowerQ.match(/(\d+)\s*(villes?|communes?|destinations?)/i) ||
...
function formatResponse(
    ...
    const intro = count === 1 ? singularLabel : `${pluralLabel} ${count}`;
...
export async function queryMap2DAI(
    ...
    // ==================== REVENU ====================
        ...
        const sortedCities = Object.entries(cityData)
            .sort(([, a], [, b]) =>
                searchLowest ? a.revenu - b.revenu : b.revenu - a.revenu,
            )
            .slice(0, count);
        ...
        const { cityNames, citiesList, intro } = formatResponse(
            ...
            count,
            searchLowest
                ? "La ville avec le revenu moyen le plus faible est :"
                : "La ville avec le revenu moyen le plus élevé est :",
            searchLowest
                ? "Villes avec les revenus moyens les plus faibles (top"
                : "Villes avec les revenus moyens les plus élevés (top",
    ...
    // ==================== SURFACE ====================
        ...
        (lowerQuestion.includes("grande") &&
            (lowerQuestion.includes("ville") ||
                lowerQuestion.includes("commune"))) ||
        ...
        const searchSmallest =
            lowerQuestion.includes("petite") ||
        ...
        const { cityNames, citiesList, intro } = formatResponse(
            ...
            searchSmallest
                ...
                : "La plus grande commune (en surface) est :",
            searchSmallest
                ? "Plus petites communes (top"
                : "Plus grandes communes (top",
    ...
    // ==================== SECTEUR D'ACTIVITÉ ====================
        ...
        if (targetSector) {
            ...
            Object.values(cityData).forEach((data) => {
                sectorCounts[data.secteur] =
                    (sectorCounts[data.secteur] || 0) + 1;
            });
    ...
    // ==================== FOCUS SUR UNE VILLE ====================
        ...
        (lowerQuestion.includes("montre") &&
            (lowerQuestion.includes("ville") ||
                lowerQuestion.includes("commune"))) ||
        (lowerQuestion.includes("info") &&
            (lowerQuestion.includes("sur") || lowerQuestion.includes("de")))
    ) {
    ...
    // ==================== COMPARAISON ====================
        ...
        if (foundCities.length >= 2) {
            ...
            textResponse =
                "Pour comparer, mentionnez deux villes. Ex: 'Compare Nice et Cannes'";
        }
    }
    ...
    // ==================== STATISTIQUES GÉNÉRALES ====================
        ...
        const totalPopulation = Object.values(cityData).reduce(
            (sum, city) => sum + city.population,
            0,
        );
        const totalEntreprises = Object.values(cityData).reduce(
            (sum, city) => sum + city.entreprises,
            0,
        );
        const avgRevenu =
            Object.values(cityData).reduce(
                (sum, city) => sum + city.revenu,
                0,
            ) / Object.keys(cityData).length;
        const avgEmploi =
            Object.values(cityData).reduce(
                (sum, city) => sum + city.emploi,
                0,
            ) / Object.keys(cityData).length;
