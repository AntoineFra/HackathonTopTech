...
export function AIResponseDisplay({ response }: AIResponseDisplayProps) {
  const confidenceColor = 
    response.confidence >= 0.8 ? 'text-green-600' : 
    response.confidence >= 0.6 ? 'text-yellow-600' : 
    'text-red-600';

  const confidenceIcon = 
    response.confidence >= 0.8 ? CheckCircle2 : 
    response.confidence >= 0.6 ? Info : 
    AlertCircle;
    ...
    <div className="space-y-6">
          ...
          <div className="flex items-start justify-between">
            ...
            <div className="flex items-center gap-2">
              ...
              <Badge variant="outline" className={confidenceColor}>
                {Math.round(response.confidence * 100)}% de confiance
              </Badge>
            </div>
          </div>
          <CardDescription>
      ...
      {response.indicators && response.indicators.length > 0 && (
        <div className="space-y-4">
          <h3 className="text-lg font-semibold">Indicateurs associés</h3>
