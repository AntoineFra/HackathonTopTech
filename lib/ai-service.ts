...
export async function queryAI(question: string): Promise<AIResponse> {
    ...
    const { sampleIndicators, searchIndicators } = await import(
        "./sample-data"
    );
    ...
    if (
        ...
        relevantIndicators = sampleIndicators.filter(
            (ind) =>
        ...
        lowerQuestion.includes("tourism") ||
        lowerQuestion.includes("tourisme")
    ) {
        relevantIndicators = sampleIndicators.filter(
            (ind) => ind.category === "Tourisme",
        );
        ...
        lowerQuestion.includes("pib")
    ) {
        relevantIndicators = sampleIndicators.filter(
            (ind) => ind.category === "Économie",
        );
        ...
        if (relevantIndicators.length > 0) {
            ...
            answer =
                "Je n'ai pas trouvé de données spécifiques correspondant à votre question. Essayez de poser des questions sur :\n- Les statistiques de population\n- L'emploi et l'économie\n- Les données touristiques\n- Les indicateurs d'infrastructure";
        }
    }
    ...
    return {
        ...
        limitations:
            "Utilise actuellement des données d'exemple pour la démonstration. Le service IA doit être connecté pour une utilisation en production.",
    };
}
